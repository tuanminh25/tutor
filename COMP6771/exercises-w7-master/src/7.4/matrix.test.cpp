...
| Operation           | Type Trait                                 | Checks if...                      |
| ------------------- | ------------------------------------------ | --------------------------------- |
| Default constructor | `std::is_default_constructible<T>`         | `T()` is valid                    |
|                     | `std::is_nothrow_default_constructible<T>` | `T()` is `noexcept`               |

| Copy constructor    | `std::is_copy_constructible<T>`            | `T(const T&)` is valid            |
|                     | `std::is_nothrow_copy_constructible<T>`    | `T(const T&) noexcept`            |

| Move constructor    | `std::is_move_constructible<T>`            | `T(T&&)` is valid                 |
|                     | `std::is_nothrow_move_constructible<T>`    | `T(T&&) noexcept`                 |

| Copy assignment     | `std::is_copy_assignable<T>`               | `T& operator=(const T&)` is valid |
|                     | `std::is_nothrow_copy_assignable<T>`       | `T& operator=(const T&) noexcept` |

| Move assignment     | `std::is_move_assignable<T>`               | `T& operator=(T&&)` is valid      |
|                     | `std::is_nothrow_move_assignable<T>`       | `T& operator=(T&&) noexcept`      |

| Destructor          | `std::is_destructible<T>`                  | `~T()` is valid                   |
|                     | `std::is_nothrow_destructible<T>`          | `~T() noexcept`                   |

Reminder a way to test const:

CHECK(std::is_invocable_v<decltype(&Class::member_function), const Class>);
